...
export const generateReportPrompt = (sessionData: SessionData): string => {
    ...
    const analysisData = {
        ...
        passages: passageResults.map(p => {
            // Find the successful attempt or the last attempt for this passage
            const passageAttempts = attempts.filter(a => a.passage_index === p.passage_index);
            const successfulAttempt = passageAttempts.find(a => a.is_correct === 1);
            const lastAttempt = passageAttempts[passageAttempts.length - 1];
            const relevantAttempt = successfulAttempt || lastAttempt;

            let readingSummary = null;
            if (relevantAttempt && relevantAttempt.reading_summary) {
                try {
                    readingSummary = JSON.parse(relevantAttempt.reading_summary);
                } catch (e) {
                    // Ignore parse error
                }
            }
            ...
            return {
                ...
                reading_summary: readingSummary // Include granular data
            };
        }),
